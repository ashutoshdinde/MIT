        ...
        {selectedSprite ? (
          <>
            <div className='flex gap-4'>
              <h2 className="text-2xl flex font-bold text-red-800 mb-4">Actions for Sprite:
              </h2>
              <div className='text-sm'><SpriteImage spriteName={selectedSprite.name} styles={{ width: "50px", height: "50px" }} /></div>
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-red-700 mb-2">Actions:</h3>
              <ul className="space-y-2">
                {selectedSprite.actions.map((action, index) => (
                  <li key={index} className="bg-grey-100 text-black-800 px-3 py-1 flex justify-between align-middle rounded-full text-sm">
          ...
          <p className="text-black-600 italic mb-6">No sprite selected</p>
        )}
      </div>
    </div>
  );
